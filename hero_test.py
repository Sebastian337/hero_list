...
def convert_to_runic(skills_list, runic_mapping):
    runic_skills = []
    for skill in skills_list:
        runic_skill = "".join([runic_mapping.get(char, char) for char in skill])
...
def main():
    ...
    fake = Faker("ru_RU")
    runic_skills = convert_to_runic(SKILLS, RUNIC)
    for i in range(1, 11):
        character = generate_character(fake, runic_skills)
        output_filename = f"cards/result_{i}.svg"
        file_operations.render_template(
            "charsheet.svg", output_filename, character)
